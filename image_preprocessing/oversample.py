...
def makeTiles(tile_size, demag, png, blank_frac, rgb_cutoff, val_frac, osr, folder, hdf5_file):
    ...
    for v in validation_set:
        ...
        if v[2] == 1:
            LR = np.asarray(np.fliplr(tile))
            UD = np.asarray(np.flipud(tile))
            ninety = np.asarray(np.fliplr(UD))
            val_storage.append(LR[None])
            val_storage.append(UD[None])
            val_storage.append(ninety[None])
            val_labels.append(1)
            val_labels.append(1)
            val_labels.append(1)
    ...
    for t in training_set:
        ...
        if t[2] == 1:
            ...
            UD = np.asarray(np.flipud(tile))
            ninety = np.asarray(np.fliplr(UD))
            train_storage.append(LR[None])
            train_storage.append(UD[None])
            train_storage.append(ninety[None])
